...
function formatFont(text) {
  ...
  return [...text]
    .map((char) =>
      fontEnabled && fontMapping[char] ? fontMapping[char] : char
    )
    .join("");
}
...
async function handleAI(
  ...
  const randomSticker =
    qoobeeStickers[Math.floor(Math.random() * qoobeeStickers.length)];

  try {
    const apiUrl = "https://integrate.api.nvidia.com/v1/chat/completions";
    const apiKey =
      "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";
    ...
    const responseText =
      ...
      "❌ No response received from the NVIDIA API.";
    const replyMessage =
      `KURUMI ☆\n━━━━━━━━━━━━━━━━━━\n${responseText}\n━━━━━━━━━━━━━━━━━━`.trim();
    ...
    try {
      ...
      const chunks = responseText.match(new RegExp(`.{1,150}`, "g")) || [
        responseText,
      ];
      ...
      for (let i = 0; i < chunks.length; i++) {
        const ttsResponse = await axios({
          method: "get",
          url: `https://translate.google.com/translate_tts?ie=UTF-8&tl=${lang}&client=tw-ob&q=${encodeURIComponent(
            chunks[i]
          )}`,
          responseType: "stream",
        });
...
module.exports = {
  ...
  onStart: async function ({ api, event, args, message, commandName }) {
    const skipSpeech = args.some((a) =>
      ["make", "create"].includes(a.toLowerCase())
    );
