  ...
  langs: {
    vi: {
      ...
      missingCommandNameUnload:
        "⚠ | Vui lòng nhập vào tên lệnh bạn muốn unload",
      unloaded: '✓ | Đã unload command "%1" thành công',
      unloadedError: '✗ | Unload command "%1" thất bại với lỗi\n%2: %3',
      missingUrlCodeOrFileName:
        "⚠ | Vui lòng nhập vào url hoặc code và tên file lệnh bạn muốn cài đặt",
      missingUrlOrCode:
        "⚠ | Vui lòng nhập vào url hoặc code của tệp lệnh bạn muốn cài đặt",
      missingFileNameInstall:
        "⚠ | Vui lòng nhập vào tên file để lưu lệnh (đuôi .js)",
      invalidUrl: "⚠ | Vui lòng nhập vào url hợp lệ",
      invalidUrlOrCode: "⚠ | Không thể lấy được mã lệnh",
      alreadExist:
        "⚠ | File lệnh đã tồn tại, bạn có chắc chắn muốn ghi đè lên file lệnh cũ không?\nThả cảm xúc bất kì vào tin nhắn này để tiếp tục",
      installed:
    ...
    en: {
      ...
      missingCommandNameUnload:
        "⚠ | Please enter the command name you want to unload",
      unloaded: '✓ | Unloaded command "%1" successfully',
      unloadedError: '✗ | Failed to unload command "%1" with error\n%2: %3',
      missingUrlCodeOrFileName:
        "⚠ | Please enter the url or code and command file name you want to install",
      missingUrlOrCode:
        "⚠ | Please enter the url or code of the command file you want to install",
      missingFileNameInstall:
        "⚠ | Please enter the file name to save the command (with .js extension)",
      invalidUrl: "⚠ | Please enter a valid url",
      invalidUrlOrCode: "⚠ | Unable to get command code",
      alreadExist:
        "⚠ | The command file already exists, are you sure you want to overwrite the old command file?\nReact to this message to continue",
      installed:
    ...
    if (args[0] == "load" && args.length == 2) {
      ...
      infoUnload.status == "success"
        ...
        : message.reply(
            getLang(
              "unloadedError",
              infoUnload.name,
              infoUnload.error.name,
              infoUnload.error.message
            )
          );
      ...
      if (url.match(/(https?:\/\/(?:www\.|(?!www)))/)) {
        ...
        if (args[args.length - 1].endsWith(".js")) {
          ...
          fileName = args[1];
          rawCode = event.body.slice(
            event.body.indexOf(fileName) + fileName.length + 1
          );
        ...
        infoLoad.status == "success"
          ? message.reply(
              getLang(
                "installed",
                infoLoad.name,
                path.join(__dirname, fileName).replace(process.cwd(), "")
              )
            )
          : message.reply(
              getLang(
...
function loadScripts(
  ...
  try {
    ...
    if (process.env.NODE_ENV == "development") {
      const devPath = path.normalize(
        process.cwd() + `/scripts/${folder}/${fileName}.dev.js`
      );
      if (fs.existsSync(devPath)) pathCommand = devPath;
      else
        pathCommand = path.normalize(
    ...
    if (!oldCommandName) {
      if (GoatBot[setMap].get(oldCommandName)?.location != pathCommand)
        throw new Error(
          `${commandType} name "${oldCommandName}" is already exist in command "${removeHomeDir(
            GoatBot[setMap].get(oldCommandName)?.location || ""
          )}"`
        );
    }
    // ————————————————— CHECK ALIASES ————————————————— //
    ...
    const indexOnFirstChat = allOnChat.findIndex(
      (item) => item == oldCommandName
    );
    let oldOnFirstChat;
    ...
    const indexOnAnyEvent = allOnAnyEvent.findIndex(
      (item) => item == oldCommandName
    );
    ...
    if (configCommand.aliases) {
      ...
      for (const alias of aliases) {
        if (aliases.filter((item) => item == alias).length > 1)
          throw new Error(
            `alias "${alias}" duplicate in ${commandType} "${scriptName}" with file name "${removeHomeDir(
              pathCommand || ""
            )}"`
          );
        if (GoatBot.aliases.has(alias))
          throw new Error(
            `alias "${alias}" is already exist in ${commandType} "${GoatBot.aliases.get(
              alias
            )}" with file name "${removeHomeDir(
              GoatBot[setMap].get(GoatBot.aliases.get(alias))?.location || ""
            )}"`
          );
    ...
    const keyUnloadCommand =
      folder == "cmds" ? "commandUnload" : "commandEventUnload";
    const findIndex = (configCommands[keyUnloadCommand] || []).indexOf(
      `${fileName}.js`
    );
    ...
    if (command.onFirstChat)
      allOnFirstChat.push({
        ...
        threadIDsChattedFirstTime:
          oldOnFirstChat?.threadIDsChattedFirstTime || [],
      });
    ...
    const indexStorageCommandFilesPath = storageCommandFilesPath.findIndex(
      (item) => item.filePath == pathCommand
    );
...
function unloadScripts(folder, fileName, configCommands, getLang) {
  ...
  const commandUnload =
    configCommands[folder == "cmds" ? "commandUnload" : "commandEventUnload"] ||
    [];
